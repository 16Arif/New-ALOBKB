...
@push('style')
    ...
    <style>
        ...
        .fault-label {
            ...
            /* Warna abu-abu gelap sesuai garis */
        ...
        fetch('/fault/sesar_indonesia.geojson')
            ...
            .then(data => {
                faultLayer = L.geoJSON(data, {
                    ...
                    onEachFeature: function(feature, layer) {
                        layer.namaSesar = feature.properties.name || "";

                        // Fungsi tambahan untuk menghitung panjang garis (opsional tapi sangat membantu)
                        // Teks hanya akan muncul jika garis sesar dirasa cukup panjang agar tidak sesak
                        const koordinat = feature.geometry.coordinates;
                        if (koordinat.length > 2) {
                            if (map.getZoom() >= zoomThreshold && layer.namaSesar) {
                                tampilkanTeksSejajar(layer);
                            }
                        }
                    }
                }).addTo(map);

                // 3. Logika otomatis: Muncul/Hilang saat User melakukan Zoom
                map.on('zoomend', function() {
                    let currentZoom = map.getZoom();

                    faultLayer.eachLayer(function(layer) {
                        if (currentZoom >= zoomThreshold && layer.namaSesar) {
                            tampilkanTeksSejajar(layer);
                        } else {
                            // Menghapus teks saja (garis tetap ada)
                            layer.setText(null);
                        }
                    });
                });
            })
            .catch(error => console.error('Gagal memuat file sesar:', error));

        // 4. Fungsi sakti untuk membuat teks sejajar garis & ANTI-TERBALIK
        function tampilkanTeksSejajar(layer) {
            // Matikan teks lama jika ada untuk menghindari penumpukan
            layer.setText(null);

            layer.setText(layer.namaSesar, {
                repeat: false, // KUNCI UTAMA: Teks hanya muncul 1x per segmen garis
                center: true, // Menempatkan teks tepat di tengah panjang garis
                offset: -10, // Jarak di atas garis
                orientation: 'flip', // Mencegah tulisan terbalik (efek cermin)
                attributes: {
                    'font-weight': 'bold',
                    'font-size': '12px',
                    'fill': '#393E46',
                    'style': 'text-shadow: 1px 1px 2px white; letter-spacing: 1px;'
                }
            });
        }
    </script>
